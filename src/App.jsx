      ...
      return (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-50 flex items-center justify-center p-4 animate-fade-in">
             <div className="bg-slate-800 p-8 rounded-2xl max-w-md w-full text-center border border-green-500/50">
                 ...
                 <p className="text-gray-400 mb-6">Order #BV-{Math.floor(Math.random()*9000)}-X sent to seller.</p>
    ...
    return (
        <div className="min-h-screen pt-32 px-4 max-w-xl mx-auto text-center">
            ...
            <form onSubmit={doSearch} className="mb-12 relative">
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="BV-XXXX-X" className="w-full bg-slate-800 p-4 rounded-xl text-white text-center text-xl tracking-widest uppercase border border-slate-600" />
                <button className="absolute right-2 top-2 bottom-2 bg-blue-600 px-6 rounded-lg text-white font-bold hover:bg-blue-500">Find</button>
            </form>
            {result === 'found' && (
...
const BergvaultSite = () => {
  ...
  const NavLink = ({ t, v }) => <button onClick={() => { setView(v); setMobileMenuOpen(false); }} className="text-gray-300 hover:text-white font-medium text-sm uppercase">{t}</button>;
  ...
  const NavWrapper = () => (
    <nav className="fixed top-0 left-0 right-0 z-40 bg-slate-900 border-b border-slate-800 h-20 flex items-center px-4">
        ...
        <div className="hidden md:flex gap-6"><NavLink t="Shop" v="home" /><NavLink t="Admin" v="admin" /></div>
        <button onClick={() => setView('home')} className="md:hidden text-white font-bold text-sm">BACK HOME</button>
        </div>
    </nav>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-900 font-sans selection:bg-blue-500 selection:text-white">
      ...
      {/* MOBILE MENU */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-slate-900 z-50 pt-24 px-4 flex flex-col gap-6">
           <button className="absolute top-6 right-4 text-white" onClick={() => setMobileMenuOpen(false)}><X /></button>
           ...
           <button onClick={() => {setShowPartnerModal(true); setMobileMenuOpen(false)}} className="text-2xl font-bold text-blue-400 text-left">Become a Partner</button>
        </div>
      )}
